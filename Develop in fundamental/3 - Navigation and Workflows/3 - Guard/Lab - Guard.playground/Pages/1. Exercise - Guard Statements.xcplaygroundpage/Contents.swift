...
calculateArea(x: 5, y: 7)
calculateArea(x: -3, y: 8)
...
func add(_ a: Int?, _ b: Int?) -> Int? {
    guard let unwrapA = a, let unwrapB = b else {
        return nil
    }
    return unwrapA + unwrapB
}
...
if let user = createUser() {
    print("First Name: \(user.firstName), Last Name: \(user.lastName), Age: \(user.age)")
} else {
    print("Failed to create user.")
}
